package com.tudou.net{
	...
	/**
	 * SWF加载器
	 * - 加载外部swf、swz文件 如果播放器版本为11一下则解压swz
	 * 
	...
	public class SWFLoader extends EventDispatcher {
		...
		public function load(url:*, context:LoaderContext = null):void
		{
			...
			if (url is ByteArray)
			{
				var bytes:ByteArray = decode(url);
				loadBytes(bytes, _context);
				return;
			}
			
			throw new IllegalOperationError("SWFLoader暂不支持载入类型" + typeof(url));
		...
		private function loadBinary(request:URLRequest):void
		{
		...
		{
			var bytes:ByteArray = decode(urlLoader.data);
			loadBytes(bytes, _context);
		}
		...
		private function loadedHandler(evt:Event):void
		{
			...
			if (loaderInfo)
			{
				if (_contentLoaderInfo == null) _contentLoaderInfo = loaderInfo;
		...
		private function decodeLzipFile(data:ByteArray):ByteArray{
